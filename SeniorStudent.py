...
class Run_env(object):
    def __init__(self, envs, agent, n_steps=2000, n_cores=2, gamma=0.99, lam=0.95, action_space_path='/content/drive/My Drive/l2rpn/ActionSpace'):
        self.envs = envs
        self.agent = agent
        ...
        self.rec_rewards = []
        self.worker_alive_steps = np.zeros(2)
    ...
    def run_n_steps(self, n_steps=None):
        ...
        # mb_values, mb_dones, mb_neg_log_p =  [],[],[]

        # # start sampling                                      # obs_objs = self.envs.get_obs()
        # obs_objs = self.envs.get_obs()                              # obss = np.asarray([obs.to_vect()[self.chosen] for obs in obs_objs])  # (12, 1221,)
        # obss = obs_objs.to_vect()[self.chosen]
        # dones = np.asarray([False])                           # (12,)
        # agent_step_rs =  np.asarray([0] , dtype = np.float64) # np.asarray([0 for _ in range(NUM_CORE)], dtype=np.float64)  # (12,)

        # for _ in range(self.n_steps):
        #     self.worker_alive_steps += 1
        #     model_act = False

        #     actions = np.asarray([None])                      # np.asarray([None for _ in range(NUM_CORE)])  
        #     values = np.asarray([None])                       # np.asarray([None for _ in range(NUM_CORE)])
        #     neg_log_ps = np.asarray([None])                   # np.asarray([None for _ in range(NUM_CORE)])

        #     act_ = self.envs.action_space({})
            
        #     if obss[654:713].max() >= ACTION_THRESHOLD:
        #         actions, values, noe_log_ps, _ =  map(lambda x: x.detach().numpy(), self.agent.step(torch.FloatTensor(obss))) #check tensortype
                
        #         act_ = actions
        #         model_act = True

        #         if dones == False and len(mb_obs) > 0:
        #             mb_rewards.append(agent_step_rs)
        ...
        #         mb_neg_log_p.append(neg_log_ps)
        #     else:
        #         pass
            
        #     reconnect_act, reconnect_act_flag_ = self.reconnect_array(obs_objs)
            
        #     #if reconnect act present combine it with topology else only topology action is converted in grid2op format here in the shape of the 494 vectors
            
        #     if reconnect_act_flag_ == True and model_act == True:     # [1,1]
        #         action_ = self.array2action(self.actions[act_], reconnect_act)
        #     elif reconnect_act_flag_ == False and model_act == True:  # [0,1]
        #         action_ = self.array2action(self.actions[act_])
        #     elif reconnect_act_flag_ ==  True and model_act == False: # [1,0]
        #         action_ = reconnect_act                               
        #     else:                                                     # [0,0]
        #         action_ = self.envs.action_space({})
            
        #     obs_objs, rs, env_dones, infos = self.envs.step(action_)
        #     obss = obs_objs.to_vect()[self.chosen] #np.asarray([obs_objs.to_vect()[self.chosen]])
            
        #     if env_dones:
        #         # death or end
        #         self.alive_steps_record.append(self.worker_alive_steps)
        #         self.worker_alive_steps = 0
        #         if 'GAME OVER' in str(infos['exception']):
        #             dones = True
        #             mb_rewards.append(agent_step_rs - 300)  # 上一个agent step的reward
        #         else:
        #             dones = True
        #             mb_rewards.append(agent_step_rs + 500)
        #     agent_step_rs += rs           
   
        # print("mb_ob_s", mb_obs)
        # # batch to trajectory
        # if mb_obs != []:
        #     if dones:
        #         mb_dones.append(np.asarray([True]))
        #         mb_values.append(np.asarray([0]))
        #     else:
        #         mb_obs.pop()
        #         mb_actions.pop()
        #         mb_neg_log_p.pop()
        ...
        # if mb_obs != []:
        #     mb_obs_i = np.asarray(mb_obs, dtype=np.float32)
        #     mb_rewards_i = np.asarray(mb_rewards, dtype=np.float32)
        #     mb_actions_i = np.asarray(mb_actions, dtype=np.float32)
        #     mb_values_i = np.asarray(mb_values[:-1], dtype=np.float32)
        #     mb_neg_log_p_i = np.asarray(mb_neg_log_p, dtype=np.float32)
        #     mb_dones_i = np.asarray(mb_dones[:-1], dtype=np.bool)
        #     last_done = mb_dones[-1][0]
        #     last_value = mb_values[-1][0]
        #     # calculate R and A
        #     mb_advs_i = np.zeros_like(mb_values_i)
        #     last_gae_lam = 0      
            
        #     for t in range(len(mb_obs))[::-1]:
        #         if t == len(mb_obs) - 1:
        #             # last step
...
# for _ in range(self.n_steps):
            ...
            for id in range(NUM_CORE):
                if obss[id, 654:713].max() >= ACTION_THRESHOLD:
                    ...
                    print("========================================start===========================================")
                    print('\n')
                    print("NUMB_CORE",id, "-------act-------", actions[id],"-----max_rho------",obss[id, 654:713].max())
                    # print(self.actions[actions[id]][0])
                    print('\n')
                    print("=========================================END=========================================")
    ...
    def array2action(self, obs, total_array, reconnect_array):
        ...
        if len(np.where(reconnect_array == 0)[0]) == 59:            # reconnect_array is None:
            print("No reconnect action present")
            ...
            if np.any(act_lines == reconnect_line):                 # reconnect_line is subset of it
                print('\n')
                print("reconnect action line present in the topology impacted lines", act_lines, reconnect_line)
                print('\n')
                print("final_act",action)
            ...
            for i in disc_line_cooldown_zero:
                if np.any(act_lines == i):                          # disconnect line id with zero cooldown present in the action topo
                    print('\n')
                    print("disc_line action line present in the topology impacted lines", act_lines, i, "reconnect line", reconnect_line)
                    print('\n')
                    print("final_act",action)
                    return action                         
        else:                            
...
act = a_space({"set_bus":{"substations_id":[(16, sub_16_topo_action)]}})
topology_action_sub_id = 20 #act.impact_on_objects()['topology']['assigned_bus'][0]['substation']

if np.any(line_affected_sub == topology_action_sub_id):
    print("ony topo action")
'''
...
if __name__ == '__main__':
    ...
    DATA_PATH = '/content/drive/My Drive/l2rpn/training_data_track1'  # for demo only, use your own dataset
    SCENARIO_PATH = '/content/drive/My Drive/l2rpn/training_data_track1/chronics'
    dir_path = '/content/drive/My Drive/l2rpn/ckpt'
    EPOCHS = 10 #1000
    NUM_ENV_STEPS_EACH_EPOCH = 500 # 20000 larger is better
    logfile = open('/content/drive/My Drive/l2rpn/log/log_file.log','w')
    ...
    runner = Run_env(envs, agent, action_space_path='/content/drive/My Drive/l2rpn/ActionSpace')

    print("out of runner start taining")
